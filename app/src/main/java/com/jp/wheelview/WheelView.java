...
public class WheelView extends View {
     ...
     * 设置数据
     */
    private List<XZQH> dataList = new ArrayList<>();
    /**
     * 按下的坐标
     */
    private int downY;
        ...
        for (ItemObject item : itemList) {
            if (item.isSelected()) {
                if (onSelectListener != null)
                    onSelectListener.endSelect(item.id, item.mXZQH);
                break;
            }
        }
    }
    ...
    private void initData() {
        ...
        for (int i = 0; i < dataList.size(); i++) {
            ...
            itmItemObject.id = i;
            itmItemObject.itemText = dataList.get(i).getXZQHMC();
            itmItemObject.mXZQH=dataList.get(i);
            itmItemObject.x = 0;
    ...
    private void onSelectListener() {
        ...
        for (ItemObject item : itemList) {
            if (item.isSelected()) {
                onSelectListener.selecting(item.id, item.mXZQH);
            }
        }
    }
     ...
     * @param data
     */
    public void setData(List<XZQH> data) {
        this.dataList = data;
        initData();
    }
     ...
     * @param data
     */
    public void resetData(List<XZQH> data) {
        setData(data);
        invalidate();
    }
     ...
     * @return
     */
    public XZQH getItem(int index) {
        if (itemList == null)
            return null;
        return itemList.get(index).mXZQH;
    }
